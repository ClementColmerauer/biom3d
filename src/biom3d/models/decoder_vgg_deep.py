...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x:List[torch.Tensor]): # x is a list of two inputs [low_res, high_res]
        low=x[0]
        high = x[1]
        low = self.up(low)
        # print("low",low.shape)
...
#         in_planes_low = in_planes[:-1]
    ...
    def forward(self, x:List[torch.Tensor]): # x is a list of input of length=6 generated by an encoder
        deep_out = []
        out = x[-1]
        ...
        if self.use_deep: return deep_out+[out]
        return [out]
